...
import { portionHistory, type DayItem, type PortionGroup } from "@/lib/daily";
...
export default function PortionPage() {
  ...
  const [groups, setGroups] = useState<PortionGroup[]>([]);
  ...
  function fetchPage(sid: number, off: number, append: boolean) {
    setLoading(true);
    portionHistory(sid, off, PAGE)
      .then((data) => {
        if (!Array.isArray(data) || data.length === 0) {
          setHasMore(false);
          if (!append) setStatus("No portion entries found.");
          return;
        }
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(() => {
      ...
      setStudentId(sid);
      setLoading(true);
      portionHistory(sid, 0, PAGE)
        .then((data) => {
          if (cancelled) return;
          if (!Array.isArray(data) || data.length === 0) {
            setHasMore(false);
            setStatus("No portion entries found.");
            return;
          }
          setGroups(data);
          setOffset(PAGE);
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col gap-4 p-6">
      <header className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-semibold">Portion Taken</h1>
      </header>
      ...
      {groups.map((g, gi) => (
        <section key={`${g.date}-${gi}`} className="flex flex-col gap-2">
          ...
          {g.portionCoveredList.map((it, i) => (
            <article
              key={`${gi}-${it.id ?? i}`}
              className="rounded-lg border bg-white p-3 shadow-sm"
            >
              <p className="font-medium">{it.subject || "Portion"}</p>
              {it.description && (
